...
export const PreviewCanvas: React.FC<Props> = ({
  ...
  return (
    <div className="flex flex-col h-full bg-slate-950 border border-slate-800 rounded-2xl overflow-hidden shadow-2xl relative">
      {/* Header bar */}
      <div className="px-5 py-3 bg-slate-900/90 backdrop-blur border-b border-slate-800 flex items-center justify-between text-xs text-slate-300">
        <div className="flex items-center gap-2">
          <div className="flex items-center justify-center w-6 h-6 rounded-lg bg-teal-500/20 text-teal-400 font-bold border border-teal-500/30">
            9:16
          </div>
          <div>
            <span className="font-semibold text-slate-200">Vista Previa Vídeo Vertical</span>
            <span className="text-slate-500 ml-2">1080 × 1920 px (60 FPS)</span>
          </div>
        </div>

        <div className="flex items-center gap-2">
          {/* Custom background uploader */}
          <label className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg bg-slate-800 hover:bg-slate-700 text-slate-200 border border-slate-700 transition cursor-pointer text-xs">
          ...
          {customBgUrl && (
            <button
              ...
              className="px-2 py-1.5 rounded-lg bg-red-950/60 hover:bg-red-900/60 text-red-300 border border-red-800/40 text-xs transition"
            >
              Quitar Fondo
            </button>
          )}
      ...
      <div
        ref={containerRef}
        className="flex-1 relative flex items-center justify-center p-4 bg-slate-950/80 overflow-hidden select-none"
        style={{
          backgroundImage:
      ...
      >
        ...
        <div
          ref={previewRef}
          className="relative bg-slate-900 rounded-lg shadow-2xl overflow-hidden border border-slate-700/60 transition-transform duration-75"
          style={{
            width: `${scaledW}px`,
            height: `${scaledH}px`,
        ...
        >
          ...
          <div className="absolute top-3 right-3 px-2 py-1 rounded bg-black/60 backdrop-blur text-[10px] font-mono text-teal-300 border border-teal-500/30 flex items-center gap-1 z-30">
      ...
      <div className="p-4 bg-slate-900 border-t border-slate-800 flex flex-col gap-3">
        ...
        <div className="relative flex flex-col gap-1">
          ...
          <div className="relative w-full flex items-center">
            ...
            <input
              ...
              value={currentTime}
              onChange={handleSeek}
              className="w-full h-2.5 bg-slate-800 rounded-lg appearance-none cursor-pointer accent-teal-400 z-10"
            />
            ...
            <div className="absolute top-0 bottom-0 left-0 right-0 pointer-events-none flex items-center">
              {/* 1.5s marker */}
              <div
                className="absolute top-0 bottom-0 w-0.5 bg-teal-500/60 z-20"
                style={{
              ...
              {/* 6.0s marker */}
              <div
                className="absolute top-0 bottom-0 w-0.5 bg-amber-500/60 z-20"
                style={{
        ...
        <div className="flex flex-wrap items-center justify-between gap-2 sm:gap-4 pt-1 border-t border-slate-800/80">
          <div className="flex items-center gap-2">
            <button
              onClick={togglePlay}
              className="flex items-center justify-center w-9 h-9 rounded-xl bg-teal-500 hover:bg-teal-400 text-slate-950 font-bold transition shadow-lg shadow-teal-500/20"
          ...
          <div className="flex items-center gap-2 font-mono text-xs bg-slate-950 px-3 py-1.5 rounded-xl border border-slate-800">
            ...
            <span className="text-teal-300 font-bold">{currentTime.toFixed(2)}s</span>
            <span className="text-slate-600">/</span>
            <span className="text-slate-400">{config.totalDuration.toFixed(2)}s</span>
          </div>
